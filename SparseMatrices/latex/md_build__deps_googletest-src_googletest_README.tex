\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md394}{}\doxysubsubsection{Setup}\label{md_build__deps_googletest-src_googletest_README_autotoc_md394}
To build Google\+Test and your tests that use it, you need to tell your build system where to find its headers and source files. The exact way to do it depends on which build system you use, and is usually straightforward.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md395}{}\doxysubsection{Build with C\+Make}\label{md_build__deps_googletest-src_googletest_README_autotoc_md395}
...
C\+Make works by generating native makefiles or build projects that can be used in the compiler environment of your choice. You can either build Google\+Test as a standalone project or it can be incorporated into an existing C\+Make build for another project.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md396}{}\doxysubsubsection{Standalone C\+Make Project}\label{md_build__deps_googletest-src_googletest_README_autotoc_md396}
...
On Mac OS X with Xcode installed, a {\ttfamily .xcodeproj} file will be generated.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md397}{}\doxysubsubsection{Incorporating Into An Existing C\+Make Project}\label{md_build__deps_googletest-src_googletest_README_autotoc_md397}
...
Note that this approach requires C\+Make 3.\+14 or later due to its use of the {\ttfamily Fetch\+Content\+\_\+\+Make\+Available()} command.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md398}{}\doxyparagraph{Visual Studio Dynamic vs Static Runtimes}\label{md_build__deps_googletest-src_googletest_README_autotoc_md398}
...
Enabling this option will make gtest link the runtimes dynamically too, and match the project in which it is included.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md399}{}\doxysubsubsection{C++ Standard Version}\label{md_build__deps_googletest-src_googletest_README_autotoc_md399}
An environment that supports C++11 is required in order to successfully build Google\+Test. One way to ensure this is to specify the standard in the top-\/level project, for example by using the {\ttfamily set(\+C\+M\+A\+K\+E\+\_\+\+C\+X\+X\+\_\+\+S\+T\+A\+N\+D\+A\+R\+D 11)} command. If this is not feasible, for example in a C project using Google\+Test for validation, then it can be specified by adding it to the options for cmake via the {\ttfamily D\+C\+M\+A\+K\+E\+\_\+\+C\+X\+X\+\_\+\+F\+L\+A\+GS} option.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md400}{}\doxysubsection{Tweaking Google\+Test}\label{md_build__deps_googletest-src_googletest_README_autotoc_md400}
...
We list the most frequently used macros below. For a complete list, see file \href{https://github.com/google/googletest/blob/master/googletest/include/gtest/internal/gtest-port.h}{\texttt{ include/gtest/internal/gtest-\/port.\+h}}.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md401}{}\doxysubsection{Multi-\/threaded Tests}\label{md_build__deps_googletest-src_googletest_README_autotoc_md401}
...
Note\+: while the above steps aren\textquotesingle{}t technically necessary today when using some compilers (e.\+g. G\+CC), they may become necessary in the future, if we decide to improve the speed of loading the library (see \href{http://gcc.gnu.org/wiki/Visibility}{\texttt{ http\+://gcc.\+gnu.\+org/wiki/\+Visibility}} for details). Therefore you are recommended to always add the above flags when using Google\+Test as a shared library. Otherwise a future release of Google\+Test may break your build script.\hypertarget{md_build__deps_googletest-src_googletest_README_autotoc_md403}{}\doxysubsection{Avoiding Macro Name Clashes}\label{md_build__deps_googletest-src_googletest_README_autotoc_md403}
